...
namespace Pathfinding
{
	...
	public class RecastGraph : NavGraph, IUpdatableGraph, IRaycastableGraph, INavmesh, INavmeshHolder, IFunnelGraph
	{
		...
		private void CollectColliderMeshes(Bounds bounds, List<ExtraMesh> extraMeshes)
		{
			...
			if ((this.tagMask != null && this.tagMask.Count > 0) || this.mask != 0)
			{
				for (int i = 0; i < array.Length; i++)
				{
					...
					if (((1 << collider.gameObject.layer & this.mask) != 0 || this.tagMask.Contains(collider.tag)) && collider.enabled && !collider.isTrigger && collider.bounds.Intersects(bounds))
					{
		...
		private bool CollectMeshes(out List<ExtraMesh> extraMeshes, Bounds bounds)
		{
			...
			{
				UnityEngine.Debug.LogWarning("No MeshFilters where found contained in the layers specified by the 'mask' variables");
				return false;
			}
			return true;
		}
